...
async function registrarComandos() {
  const modoChoices = Object.entries(MODOS_DE_JOGO).map(([key, val]) => ({
    name: val.nome,
    value: key,
  }));
  ...
  const comando = new SlashCommandBuilder()
    ...
    .addNumberOption((opt) =>
      opt.setName("valor").setDescription("Valor da aposta em reais (R$)").setRequired(true).setMinValue(0.01)
    )
    .addStringOption((opt) =>
      opt.setName("modo").setDescription("Modo de jogo da aposta").setRequired(true).addChoices(...modoChoices)
    )
    .addStringOption((opt) =>
      opt.setName("descricao").setDescription("Informações extras (mapa, regras, etc)").setRequired(false)
    );
  ...
  try {
    ...
    if (err.code === 50001) {
      console.error(`❌ Sem permissão. Link de convite: https://discord.com/api/oauth2/authorize?client_id=${CONFIG.CLIENT_ID}&permissions=536874048&scope=bot%20applications.commands`);
    } else {
...
function criarEmbedAposta(user, valor, modo, descricao, status) {
  ...
  const statusTexto = {
    ...
    aceita: "✅ Aposta Aceita — Ticket Aberto",
    cancelada: "❌ Cancelada",
    encerrada: "🏁 Encerrada",
  };
...
client.once("ready", async () => {
  ...
  console.log(`📋 Servidor alvo: ${CONFIG.GUILD_ID || "Global"}`);
  console.log(`📢 Canal de apostas: ${CONFIG.CANAL_APOSTAS_ID || "Todos os canais"}`);
  ...
  try {
    ...
    console.log("✅ Biografia do bot atualizada!");
  } catch (err) {
...
client.on("interactionCreate", async (interaction) => {
  try {
    if (interaction.isChatInputCommand() && interaction.commandName === "apostados") {
      await handleComandoApostados(interaction);
      return;
    }
    if (interaction.isButton()) {
      const parts = interaction.customId.split(":");
      const acao = parts[0];
      const apostaId = parts[1];
    ...
    console.error("Erro numa interação:", err);
    try {
...
async function handleComandoApostados(interaction) {
  if (CONFIG.CANAL_APOSTAS_ID && interaction.channelId !== CONFIG.CANAL_APOSTAS_ID) {
    return interaction.reply({ content: `❌ Este comando só pode ser usado no canal <#${CONFIG.CANAL_APOSTAS_ID}>.`, ephemeral: true });
  }
  ...
  if (!modo) return interaction.editReply({ content: "❌ Modo de jogo inválido." });
  ...
  const mensagem = await interaction.editReply({
    content: `🏆 **Nova aposta criada!** <@${user.id}> quer apostar ${formatarReais(valor)} no modo **${modo.nome}**! Alguém topa?`,
    embeds: [criarEmbedAposta(user, valor, modo, descricao, "aguardando")],
    components: [row],
  });

  apostasAtivas.set(apostaId, {
    apostadorId: user.id, apostadorTag: user.username,
    valor, modo, descricao,
    mensagemId: mensagem.id, canalId: interaction.channelId,
    guildId: interaction.guildId, criadaEm: Date.now(), status: "aguardando",
  });
  ...
  setTimeout(async () => {
    ...
    if (aposta && aposta.status === "aguardando") {
      ...
      try {
        ...
        await msg.edit({ content: "⏰ Esta aposta expirou (24 horas sem adversário).", embeds: [criarEmbedAposta(fakeUser, aposta.valor, aposta.modo, aposta.descricao, "encerrada")], components: [] });
      } catch (_) {}
...
async function handleAceitarAposta(interaction, apostaId) {
  ...
  const aposta = apostasAtivas.get(apostaId);
  if (!aposta)                        return interaction.editReply({ content: "❌ Esta aposta não existe ou já expirou." });
  if (aposta.status !== "aguardando") return interaction.editReply({ content: "❌ Esta aposta já foi aceita ou cancelada." });
  ...
  try {
    ...
    return interaction.editReply({ content: "❌ Não foi possível criar o canal de ticket. Verifique se o bot tem permissão de **Gerenciar Canais**." });
  }
  ...
  const embedTicket = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "📋 Como funciona", value: "1️⃣ Combinem aqui o horário e o servidor do jogo.\n2️⃣ Após o jogo, clique em **Eu ganhei!** e envie o print do resultado.\n3️⃣ Um árbitro confirmará e o ticket será fechado.", inline: false }
    )
    .setTimestamp()
    .setFooter({ text: "Bot Apostados Brawl Stars 🎮" });
  ...
  try {
    ...
    const fakeUser      = { id: aposta.apostadorId, username: aposta.apostadorTag, displayAvatarURL: () => null };
    await msgOriginal.edit({ content: `✅ Aposta aceita por <@${adversario.id}>! Ticket criado: ${ticketCanal}`, embeds: [criarEmbedAposta(fakeUser, aposta.valor, aposta.modo, aposta.descricao, "aceita")], components: [] });
  } catch (_) {}

  await interaction.editReply({ content: `✅ Você aceitou a aposta! O ticket foi criado em ${ticketCanal}.` });
}
...
async function handleCancelarAposta(interaction, apostaId) {
  ...
  const aposta = apostasAtivas.get(apostaId);
  if (!aposta) return interaction.editReply({ content: "❌ Esta aposta não existe ou já expirou." });
  if (!isAdmin(interaction.member) && interaction.user.id !== aposta.apostadorId) return interaction.editReply({ content: "❌ Somente o apostador ou um admin pode cancelar." });
  if (aposta.status !== "aguardando") return interaction.editReply({ content: "❌ Só é possível cancelar apostas aguardando adversário." });

  aposta.status = "cancelada";
  try {
    const fakeUser  = { id: aposta.apostadorId, username: aposta.apostadorTag, displayAvatarURL: () => null };
    const canalOrig = await client.channels.fetch(aposta.canalId);
    const msgOrig   = await canalOrig.messages.fetch(aposta.mensagemId);
    await msgOrig.edit({ content: "❌ Aposta cancelada.", embeds: [criarEmbedAposta(fakeUser, aposta.valor, aposta.modo, aposta.descricao, "cancelada")], components: [] });
  } catch (_) {}

  apostasAtivas.delete(apostaId);
  await interaction.editReply({ content: "✅ Aposta cancelada com sucesso." });
}

async function handleResultado(interaction, apostaId) {
  await interaction.deferReply({ ephemeral: false });
  const aposta = apostasAtivas.get(apostaId);
  if (!aposta) return interaction.editReply({ content: "❌ Dados desta aposta não encontrados." });

  const participa = interaction.user.id === aposta.apostadorId || interaction.user.id === aposta.adversarioId;
  if (!participa && !isAdmin(interaction.member)) return interaction.editReply({ content: "❌ Somente os participantes ou admins podem declarar o resultado." });
  ...
  await interaction.editReply({
    embeds: [new EmbedBuilder().setTitle("🏆 Resultado Declarado — Aguardando Confirmação").setColor(0xf59e0b).setDescription("⚠️ Um árbitro precisa confirmar. Envie o print do placar e aguarde.").addFields({ name: "Declarado por", value: `<@${interaction.user.id}>`, inline: true }, { name: "Vencedor alegado", value: `<@${vencedorId}>`, inline: true }, { name: "Valor em jogo", value: formatarReais(aposta.valor), inline: true }).setTimestamp()],
...
async function handleConfirmarResultado(interaction, apostaId, vencedorId) {
  await interaction.deferReply({ ephemeral: false });
  if (!isAdmin(interaction.member)) return interaction.editReply({ content: "❌ Somente árbitros/admins podem confirmar o resultado." });
  ...
  setTimeout(async () => {
    try {
      await interaction.channel.send("🔒 Este ticket será fechado automaticamente em instantes...");
...
async function handleDisputarResultado(interaction, apostaId) {
  await interaction.deferReply({ ephemeral: false });
  if (!isAdmin(interaction.member)) return interaction.editReply({ content: "❌ Somente árbitros/admins podem disputar o resultado." });
  await interaction.editReply({ content: `⚠️ **Resultado em disputa!** <@${interaction.user.id}> sinalizou inconsistência. Analise os prints e confirme manualmente.`, components: [] });
}

async function handleFecharTicket(interaction) {
  await interaction.deferReply({ ephemeral: true });
  if (!isAdmin(interaction.member)) return interaction.editReply({ content: "❌ Somente admins podem fechar o ticket manualmente." });
  await interaction.editReply({ content: "✅ Fechando o ticket em 5 segundos..." });
  setTimeout(async () => { try { await interaction.channel.delete("Ticket encerrado por admin"); } catch (err) { console.error(err); } }, 5000);
}
